...
export class MessageProcessor {
  ...
  async classifyMessage(message: WhatsAppMessage): Promise<MessageClassification> {
  ...
  private extractTextFromMessage(message: WhatsAppMessage): string {
...
{"type": "unknown", "confidence": 0.2, "reason": "Unclear message with no clear purpose"}`,
  ...
  private extractPropertyData(text: string, message: WhatsAppMessage): any {
  ...
  private extractPromotionData(text: string, message: WhatsAppMessage): any {
  ...
  private extractPropertyType(text: string): 'house' | 'apartment' | 'commercial' | 'land' {
  ...
  private extractImages(message: WhatsAppMessage): string[] {
    const images: string[] = [];
  ...
  private extractPromotionTitle(text: string): string {
    const lines = text.split('\n');
    const firstLine = lines[0] || '';
  ...
  private extractDiscount(text: string): string | undefined {
