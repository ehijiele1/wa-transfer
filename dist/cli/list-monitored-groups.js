...
async function main() {
    ...
    if (opts.unregister) {
        const ok = await groupManager_1.groupManager.unregisterGroup(opts.unregister);
        if (ok) {
    ...
    let groups;
    if (opts.activeOnly) {
        groups = await groupManager_1.groupManager.getActiveGroups();
    }
    else {
        groups = await groupManager_1.groupManager.getAllGroups();
    }
    if (opts.json) {
...
main().catch((err) => {
    logger_1.logger.error('CLI error', err);
    console.error('❌ Error:', err instanceof Error ? err.message : err);
    process.exit(1);
});
//# sourceMappingURL=list-monitored-groups.js.map